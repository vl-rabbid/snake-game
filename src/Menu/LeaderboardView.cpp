...
namespace SnakeGame
{
    ...
    void LeaderboardView::Draw(sf::RenderTexture &texture) const
    {
    ...
    void LeaderboardView::HandleInput(MenuCommand &command, const sf::Event &event)
    {
    ...
    void LeaderboardView::LoadUI(const Leaderboard &leaderboard)
    {
        firstDisplayedItem = 0;
    ...
    void LeaderboardView::LoadUIEntries()
    {
        for (int i = 0; i < LEADERBOARD_DISPLAYED; i++)
        {
            if (i + firstDisplayedItem < entries.size())
