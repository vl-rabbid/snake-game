...
namespace SnakeGame
{
    ...
    void LevelSelect::Draw(sf::RenderTexture &texture) const
    {
        for (int i = 0; i < ui.buttons.size(); ++i)
            if (i < levels.size())
    ...
    void LevelSelect::HandleInput(MenuCommand &command, const sf::Event &event)
    {
    ...
    void LevelSelect::LoadFromFiles()
    {
    ...
    const LevelConfig &LevelSelect::GetSelectedLevelConfig() const
    {
        return levels[selected];
    }

    void LevelSelect::LoadUI()
    {
        LoadUIButtons();
        SetSelector();
    ...
    void LevelSelect::LoadUIButtons()
    {
        for (int i = 0; i < ui.buttons.size(); i++)
        {
