...
namespace SnakeGame
{
    ...
    {
        InitMenus(allMenus);
        levelSelect.Init(resources);
        leaderboardView.Init(resources);
    ...
    void MenuManager::SetState(const MenuState &state, const Config &gameConfig, const Leaderboard &gameLeaderboard)
    {
        ...
        {
        case MenuState::LevelSelect:
            levelSelect.LoadFromFiles();
            break;
        case MenuState::GameOver:
            leaderboardView.LoadUI(gameLeaderboard);
            break;
        case MenuState::Leaderboard:
            leaderboard.LoadFromFile(levelSelect.GetSelectedLevelConfig());
    ...
    MenuCommand MenuManager::HandleInput(const sf::Event &event)
    {
        ...
        {
        case MenuState::Leaderboard:
            leaderboardView.HandleInput(command, event);
            break;
        case MenuState::LevelSelect:
            levelSelect.HandleInput(command, event);
            break;
        case MenuState::GameOver:
            leaderboardView.HandleInput(command, event);
            break;
    ...
    {
        return levelSelect.GetSelectedLevelConfig();
    }
    ...
    void MenuManager::DrawStates(const MenuState &state, sf::RenderTexture &texture) const
    {
        ...
        {
        case MenuState::LevelSelect:
            levelSelect.Draw(texture);
            break;
        case MenuState::Leaderboard:
            leaderboardView.Draw(texture);
            break;
        case MenuState::GameOver:
            leaderboardView.Draw(texture);
            break;
